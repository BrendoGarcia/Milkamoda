document.addEventListener("DOMContentLoaded", function() {
  const botoesAdicionar = document.querySelectorAll(".btn.btn-primary");
  ...
  const botaoFinalizarCompra = document.createElement("button");
   const botaocencelar = document.createElement("button");
   const postar = document.querySelectorAll(".btn.btn-outline-danger.btn-lg");
  ...
  botaocencelar.innerText = "Cancelar Compra";
    botaocencelar.classList.add("cencelarcompra");
  carrinho.appendChild(botaocencelar);

  // Estiliza o botão de finalizar compra

  botaocencelar.style.alignItems = "center";
  botaocencelar.style.backgroundColor = "#FF0000";
  botaocencelar.style.textAlign = "center";
  botaocencelar.style.margin = "0";
  botaocencelar.style.color = "white";
  botaocencelar.style.padding = "10px 20px";
  botaocencelar.style.border = "none";
  botaocencelar.style.borderRadius = "5px";
  botaocencelar.style.cursor = "pointer";
  botaocencelar.style.display = "none"; 
  ...
  botaocencelar.addEventListener("click", function() {
    alert("Compras Cancelada com Sucesso!!");
    limparCarrinho();
  ...
  botoesAdicionar.forEach(function(botao) {
    botao.addEventListener("click", function() {
      ...
      const quantidade = parseInt(produto.querySelector(".qtde-carrinho").value);
      calcularSubtotal();
      atualizarContadorItens()
  ...
  postar.forEach(function(botao) {
    botao.addEventListener("click", function() {
      ...
      var inputEstrelas = document.getElementById("inputEstrelas").value;

      adicionarComentario(inputUsuario, inputComentario, inputEstrelas);

      console.log("Usuario:", inputUsuario);
      console.log("Comentario:", inputComentario);
      console.log("Estrelas:", inputEstrelas);
    });
  });
  ...
  imagemCarinho.addEventListener("click", function() {
    carrinho.style.display = carrinho.style.display === "none" ? "grid" : "none";
  ...
  function calcularSubtotal() {
    ...
    botaocencelar.style.display = subtotal > 0 ? "block" : "none";
  }
...
const botoesComprar = document.querySelectorAll('.btn.btn-primary');
...
function limparCarrinho() {
    const listaItensCarrinho = document.getElementById("listaItensCarrinho");
    while (listaItensCarrinho.firstChild) {
        listaItensCarrinho.removeChild(listaItensCarrinho.firstChild);
    }
    // Após remover todos os itens, atualiza o contador de itens para 0
    atualizarContadorItens();
...
function esconderContador() {
    const contadorItens = document.querySelectorAll(".contadordeitens");
    document.getElementById("teste").innerText = "";
    contadorItens.style.display = "none";
}



function adicionarComentario(usuario, comentario, estrelas) {
  // Criar um novo elemento div para armazenar o novo comentário
  var novoComentario = document.createElement("div");
  novoComentario.classList.add("armazenarComentarios",
  "border", "border-3", "rounded", "d-flex", "align-items-center", "rounded-lg", "cursor-pointer", "shadow-sm", "bg-light");

  // Crie a estrutura interna do novo comentário
  novoComentario.innerHTML = `
      <div class="comentarios  align-items-center p-3">
          <h6>Comentário</h6>
          <p class="users">Usuário: ${usuario} </p>
          <p>${comentario}</p>
          <p>`;
  
  // Adiciona as estrelas ao lado do comentário
  for (var i = 0; i < estrelas; i++) {
      novoComentario.innerHTML += `<i class="bx bxs-star"></i>`;
  }

  novoComentario.innerHTML += `</p></div>`;

  // Adicionar o novo comentário à página
  var containerComentarios = document.getElementById("containerComentarios");
  containerComentarios.appendChild(novoComentario);
}
